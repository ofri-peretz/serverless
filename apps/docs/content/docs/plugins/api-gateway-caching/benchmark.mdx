...
All seven dimensions are continuously measured. The lifecycle-correctness dimension reads the latest E2E verdict from [`scripts/e2e/runs/`](https://github.com/ofri-peretz/serverless/blob/main/packages/serverless-api-gateway-caching/scripts/e2e/runs/) and [`scripts/e2e-community/runs/`](https://github.com/ofri-peretz/serverless/blob/main/packages/serverless-api-gateway-caching/scripts/e2e-community/runs/), and combines it with a static check for `*:remove:*` hooks and safe-offboarding commands. Live measurement of the uninstall-path scenario is tracked as the [`cleanup-uninstall-path` suite](https://github.com/ofri-peretz/agents/blob/main/interlace/serverless/evidence-plan.md) for v1.1.
...
We don't run live `sls deploy` from the benchmark itself — that happens in the E2E harness, and we read its verdict. Cache hit-rate measurement and cold-start impact are tracked as separate suites ([`cold-start`](https://github.com/ofri-peretz/serverless/tree/main/benchmarks/suites/cold-start), [`deploy-latency`](https://github.com/ofri-peretz/serverless/tree/main/benchmarks/suites/deploy-latency)) — both currently scaffolded as skeletons.
